...
// Yes it shares the YAEL include !!
...
#define LCD_CONSOLE_DEF_COLS LCD_CONSOLE_80_COLS
   ...
   void xts_yat4l_screen_consoleColorSet(uint16_t bg, uint16_t fg, uint16_t acc) {
      ::consoleColorSet(bg, fg, acc);
   }

   void xts_yat4l_screen_consoleSetMode(uint8_t columnMode, bool rerenderFull) {
   ...
   void yat4l_tft_println(char* str) { yat4l_tft_print(str); consoleWrite('\n'); }
   void yat4l_tft_print(char ch) { 
      consoleWrite(ch);
   }
  ...
  // === TFT direct routines ============
   ...
   #define FONT_TINY 1
      ...
      void __fillPixelRect(int x, int y, int w, int h, uint16_t* bgColor) {
         ...
         tft.drawRGBBitmap(x, y, bgColor, w, h);
      }
...
// **************************************************
  ...
  // ==========] Console Routines [==========
   ...
   void _doPlay(char* sequence) {
      ...
      if ( slen <= 2 ) { 
         yat4l_buzzer_playTuneString(sequence); 
         return; 
      }
      ...
      if ( (ch == '3' || ch == 'K') && sequence[ slen-2  ] == '5' ) {
         yat4l_buzzer_playTuneFile(sequence);
      }  else {
         yat4l_buzzer_playTuneString(sequence);
      }
   }
...
// #define DBUG_VT_ESC 1
  ...
  // ==========] Draw Bmp Routines [==========
   ...
   void drawBmp(char* filename, bool screenRotate) {
      ...
      yat4l_tft_drawBMP(filename, 0, 0);
   }
   ...
   void drawImgFromPAK(char* filename, int x, int y, int numInPak) {
      yat4l_tft_drawPAK(filename, x, y, numInPak);
   }
