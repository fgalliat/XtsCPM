...
// 	void delay(int time) { usleep(time*1000); }
  ...
  // =============================
    ...
    char** _snd_trackNames = NULL;
 	...
 	// SoundCard::SoundCard(SoftwareSerial* serial) { this->serial = serial; }
 	// SoundCard::SoundCard(HardwareSerial* serial) { this->serial = serial; }
    ...
    void SoundCard::play(int track) {
    	...
    	_snd_trackNum = track;
    	execute_CMD(this->serial, 0x03, (uint8_t)(track>>8) , (uint8_t)(track%256));  // play track 1 from SDCARD 
		delay(100);
    }
